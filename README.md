...
| `COMFYUI_MCP_PANEL_MCP_PORT` | bridge port + 1 | Streamable-HTTP port for the tab-scoped `panel_*` live-canvas tools. |
| `COMFYUI_MCP_PANEL_MCP_HOST` | `127.0.0.1` | Bind host for external MCP clients such as Codex on another machine. Set exactly `0.0.0.0` for LAN access; remote mode fails closed unless both `…_PUBLIC_URL` and a strong `…_TOKEN` are set. Internal panel agents continue to use the credential-free loopback route. |
| `COMFYUI_MCP_PANEL_MCP_PUBLIC_URL` | | Externally reachable base URL, e.g. `http://192.168.1.12:9181`. Required for a non-loopback panel-MCP bind and advertised to the Agent panel; no token is included. |
| `COMFYUI_MCP_PANEL_MCP_TOKEN` | | Bearer secret for non-loopback `panel_*` requests (minimum 24 characters). Set the same variable in the external Codex process, then use the Agent panel's **Settings → Advanced → Use this canvas from Codex** command. |
