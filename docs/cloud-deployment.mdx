...
**Dead-man switch (v0.47+).** Idle auto-stop lives in the comfyui-mcp process —
if that process dies (crash, closed laptop), the backstop used to die with it
and the pod billed forever. Pods **created through the connector** now carry a
pod-side watchdog: while comfyui-mcp minds the pod it heartbeats every few
seconds; if the beats stop, the pod **stops itself** (never terminates — your
...
To finish, with `connect` still running on your own machine, open the pod's ComfyUI in
your browser, open the **Agent Panel** sidebar, and click **Connect**.
