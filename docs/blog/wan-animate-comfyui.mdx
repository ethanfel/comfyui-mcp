---
title: "WAN Animate 2.2 in ComfyUI: Character Replace"
...
> installer), then drive the graph from your own Claude session via the Panel. The
> shipped graph asks for SageAttention — if you don't have it, set `attention_mode`
> to `sdpa` and skip the toolchain entirely. Jump to
...
**WAN Animate 2.2 is Apache 2.0** — fully permissive, commercial use included.
[Wan-AI/Wan2.2-Animate-14B](https://huggingface.co/Wan-AI/Wan2.2-Animate-14B) states
it in the model card: the models in the repository are licensed under Apache 2.0.

That is rare in the video-model space and is a large part of why this model got
adopted so fast against the closed Runway and ByteDance options — you can put
character replacement into a commercial pipeline without negotiating anything.

The detection and segmentation models the pack pulls alongside it (SeC, ViTPose,
YOLO) are separate projects with their own terms; check those if you redistribute
the bundle rather than just using it.
...
> **The shipped graph asks for SageAttention.** `WanVideoModelLoader` is set to
> `attention_mode: sageattn`, which needs `triton-windows` plus a matching
> SageAttention wheel in the Python that runs ComfyUI (on Windows portable, that is
> `...\ComfyUI_windows_portable\python_embeded\`). **This pack does not install
> them**, and it does not ship a script that does — if you have seen
> `install_triton_and_sageattention_auto.bat` referenced, that is a third-party
> community helper, not something included here.
>
> **You do not have to install either.** Set `attention_mode` to **`sdpa`** on the
> `WanVideoModelLoader` node and the graph runs on stock PyTorch attention — slower
> per step, no Triton, no wheel matching. That is a supported value on this exact
> node; the [`wan-pusa-extend`](https://github.com/artokun/comfyui-mcp/tree/main/packs/wan-pusa-extend)
> pack ships `sdpa` for the same reason. Switch to `sageattn` later if you want the
> speed and have the toolchain working.
...
- **Triton/SageAttention errors at sampling.** Set `attention_mode` to `sdpa` on the
  `WanVideoModelLoader` node and restart — that removes the dependency entirely and
  costs only speed. Chasing a working Triton + SageAttention wheel pair is the harder
  path, and nothing in this pack installs them for you.
...
3. Restart and load `workflow.json`. If you don't have Triton + SageAttention set up, set `attention_mode` to `sdpa` on the `WanVideoModelLoader` node — slower, but no toolchain to install.
