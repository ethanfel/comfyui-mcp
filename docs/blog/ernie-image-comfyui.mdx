---
title: "Run ERNIE-Image Locally in ComfyUI (Apache-2.0)"
...
**ERNIE-Image is Apache 2.0** — commercial use, modification and redistribution, full
stop. No revenue threshold, no seat count, no separate agreement. Both
[baidu/ERNIE-Image](https://huggingface.co/baidu/ERNIE-Image) and
[baidu/ERNIE-Image-Turbo](https://huggingface.co/baidu/ERNIE-Image-Turbo) declare
`apache-2.0`, and the Comfy-Org repackage carries the same terms.

This is the post's headline claim and it holds up. It is a real difference from
[Ideogram 4](./ideogram-4-comfyui), whose weights are open but **non-commercial**.

If you install the monolith `ernie` pack, note that its combo groups also load
Z-Image-Turbo, which is separately Apache 2.0 — so the combined pipeline stays
commercially usable.
...
their headers literally say "Z-IMAGE-BASE" and they download Z-Image files
(`z_image_turbo-*.gguf`, `Qwen3-4B-*.gguf`, `ae.safetensors`) while wiring **none**
of them. You pay the disk and can end up pointing at the wrong VAE.

Which pack you want depends on whether you actually want the combos:

| pack | pulls Z-Image models? | why |
|---|---|---|
| **`ernie-txt2img`** | **no** | pure ERNIE text-to-image — this is the "ERNIE-only" install |
| **`ernie-img2img`** | **no** | pure ERNIE image-to-image |
| `ernie` (monolith) | **yes**, deliberately | 10 of its 20 groups are ERNIE↔Z-Image combo pipelines, and they load those models |
| `ernie-combo` | **yes**, deliberately | it *is* the combo pipeline |

The difference from the third-party installer isn't the file list — it's that when a
comfyui-mcp pack downloads `z_image_turbo-Q8_0.gguf`, a group in the graph loads it.
Nothing lands unused. If you want ERNIE and nothing else, take `ernie-txt2img` rather
than the monolith.
...
every file it downloads is one the graph loads — and the same manifest drives both an
MCP-native install and the generated double-click scripts:
...
- **Installer pulled Z-Image files the graph never loads** → that's the contaminated
  upstream script. Use `ernie-txt2img` (or `ernie-img2img`) if you want ERNIE and
  nothing else; the `ernie` monolith downloads Z-Image models on purpose, because its
  combo groups load them.
...
2. Apply the **`ernie` pack** — nodes + models land in the right folders, validated, and every file it downloads is one the graph loads. Want ERNIE and nothing else, without the combo pipelines? Use **`ernie-txt2img`**.
