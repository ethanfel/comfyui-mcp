---
title: "Z-Image in ComfyUI: The Low-VRAM Speed King"
...
**Z-Image is Apache 2.0** — commercial use included, no revenue threshold, no
separate agreement.
[Tongyi-MAI/Z-Image-Turbo](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo) declares
`apache-2.0`.

Together with its VRAM footprint that is the practical argument for Z-Image: it is
the model you can both run on a small card and ship commercially without reading a
threshold table. Compare [Ideogram 4](./ideogram-4-comfyui), which is stronger on
text layout but **non-commercial**.
...
**Is Z-Image open source?** Yes — [Tongyi-MAI/Z-Image-Turbo](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo)
declares **Apache 2.0**, so commercial use is permitted with no revenue threshold.
See [Licensing](#licensing) above.
