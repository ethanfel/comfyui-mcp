...
> **Open weight, not Apache.** LTX-2 ships under the
> [**LTX-2 Community License Agreement**](https://huggingface.co/Lightricks/LTX-2/blob/main/LICENSE),
> not Apache 2.0 — open weights, but commercial use requires a separate paid
> license once your annual revenue reaches **$10,000,000 USD**. Permissive for
> almost everyone, and worth knowing about before you build a business on it.
...
- **LTX-2 / LTX-V2** (early 2026) — the ~19B-class DiT audio-video foundation
  ...
  audio and video in one pass, up to native **4K @ 50fps** and ~20s clips, under
...
**LTX-2 is open-weight, not Apache.** It ships under the **LTX-2 Community License
Agreement**, which permits commercial use only while your **annual revenue is under
$10,000,000 USD**. Above that you need a separate license from Lightricks.
[Lightricks/LTX-2](https://huggingface.co/Lightricks/LTX-2) declares
`license_name: ltx-2-community-license-agreement`, and the repo's LICENSE file is
that agreement — not Apache 2.0.

For most individuals and small studios that is effectively permissive. For anyone
inside a larger company it is a real gate, and the threshold counts the company, not
the team.

LTX-2.3's exact terms are worth confirming on the official Lightricks repo before
commercial use, and the community "sulphur" finetune is a separate third-party
...
| File | Folder |
|------|--------|
| `ltx-2.3-22b-dev.safetensors` (22B dev transformer + audio VAE, ~46 GB) | `models/checkpoints/` |
| `gemma_3_12B_it_fp8_scaled.safetensors` (text encoder) | `models/text_encoders/` |
| `ltx_2.3_22b_distilled_1.1_lora_dynamic_fro09_avg_rank_111_bf16.safetensors` (fast path) | `models/loras/` |
...
<Note>
  Tight on VRAM? The 22B checkpoint has an **fp8 build at roughly half the size**
  (~23 GB) at [Lightricks/LTX-2.3-fp8](https://huggingface.co/Lightricks/LTX-2.3-fp8) —
  swap it in at the same path.
</Note>
...
**Is it really open / free?** Open weights, but not Apache. LTX-2 shipped under the
**LTX-2 Community License Agreement**, which permits commercial use only while your
annual revenue is under **$10,000,000 USD**. LTX-2.3's exact
