...
Both halves of this pipeline are **Apache 2.0**, which is unusually clean for a
video stack:

- **WAN 2.2 T2V A14B** — the base model — is Apache 2.0 per
  [Wan-AI/Wan2.2-T2V-A14B](https://huggingface.co/Wan-AI/Wan2.2-T2V-A14B).
- **Pusa V1** — the LoRAs that make temporal extension work — is Apache 2.0 per
  [RaphaelLiu/PusaV1](https://huggingface.co/RaphaelLiu/PusaV1).

So commercial use is permitted on both, with no revenue threshold and no separate
agreement. The fp8 checkpoints and LoRA conversions the pack downloads are
redistributions of those weights via kijai's ComfyUI repos.
...
Pusa has a dedicated pack —
**[`wan-pusa-extend`](https://github.com/artokun/comfyui-mcp/tree/main/packs/wan-pusa-extend)** —
and it is self-contained: the WanVideoWrapper + KJNodes + VideoHelperSuite nodes, the
WAN 2.2 T2V A14B HIGH/LOW **fp8** checkpoints, both Pusa V1 LoRAs, the umt5-xxl
encoder, the WAN 2.1 VAE, and the lightx2v distill LoRA. Nothing to place by hand.

1. Install [comfyui-mcp](https://github.com/artokun/comfyui-mcp) and the
   [Panel](../panel) — then start the agent with `npx -y comfyui-mcp@latest connect`
   and click **Connect** in the panel (no API keys; sign in with `claude` once).
2. Apply the pack: `apply_manifest --path packs/wan-pusa-extend/manifest.yaml`. It
   lands the nodes and every model above in the right folders, and ships the graph
   pre-wired — `flowmatch_pusa`, `merge_loras=false`, the ColorMatchV2 seam regrade,
   and `attention_mode=sdpa` so it runs without SageAttention/triton.
3. Open the [Panel](../panel) and drive the graph from your own Claude session — load
   your clip, tune the noise ramp, iterate on the tail conversationally, with full
   undo and no extra API keys.

<Warning>
Don't substitute the `wan-longer-videos` pack here. It ships the **GGUF** A14B experts
for the ComfyUI-GGUF loader stack; the Pusa graph is built on WanVideoWrapper and wants
the **fp8 `..._KJ.safetensors`** checkpoints. They are different files on a different
node stack — mixing them leaves you re-pointing every loader by hand.
</Warning>
