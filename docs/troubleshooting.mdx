...
- Turn on [compact tool mode](./local-llms) for small models — it is **not**
  the default, so start the server with `--compact` (or
  `COMFYUI_MCP_TOOL_MODE=compact`). Without it the full schema surface
  overflows a small context and the model starts hallucinating tool names.
